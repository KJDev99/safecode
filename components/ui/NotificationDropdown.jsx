...
export default function NotificationDropdown({
    ...
    const patchDataToken = async (url, data = {}) => {
        try {
            ...
            const response = await fetch(`https://api.safecode.flowersoptrf.ru${url}`, {
                method: 'PATCH',
                headers: {
    ...
    const loadServerNotifications = async () => {
        setIsLoading(true);
        try {
            const response = await getDataToken(`/api/v1/notification/?is_read=true`);
    ...
    const handleMarkAsRead = async (notificationId) => {
        try {
            const result = await patchDataToken(`/api/v1/notification/${notificationId}/read/`, {});
                        ...
                        <div className="max-h-96 overflow-y-auto">
                            {isLoading ? (
                                ...
                                <div className="p-8 text-center">
                                    ...
                                    <p className="text-gray-400 text-sm mt-1">Здесь появятся новые сообщения</p>
                                </div>
                            ) : (
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex justify-between items-start">
                                                        ...
                                                        <span className="text-xs text-gray-400 whitespace-nowrap ml-2">
                                                            {formatTime(notification.created_at || notification.timestamp || new Date())}
                                                        </span>
                                                    </div>
                                                    ...
                                                    {!notification.read && (
                                                        <div className="flex items-center gap-2 mt-2">
                                                            ...
                                                            <span className="text-xs text-blue-600 font-medium">Новое</span>
                                                        </div>
                                                    )}
                                                </div>
