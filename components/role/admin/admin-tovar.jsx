...
export default function AdminTovar() {
    const { data, loading, getDataToken, postDataToken, putDataToken, deleteDataToken } = useApiStore();
    ...
    useEffect(() => {
        if (data) {
            ...
            if (window.location.href.includes('/products/?page=')) {
    ...
    const deleteImageFromServer = async (imageId) => {
        try {
            const response = await deleteDataToken(`/products/images/${imageId}/delete/`);
    ...
    const handleCreateProduct = async () => {
        if (!formData.name || !formData.article || formData.price <= 0 || !formData.category) {
        ...
        try {
            ...
            if (response.ok && (result.success || result.id)) {
                ...
                toast.error(result?.message || result?.error || 'Ошибка при создании товара');
    ...
    const handleEditProduct = async () => {
        ...
        try {
            ...
            const newImages = formData.images_list.filter(img => img instanceof File);
            ...
            if (response.ok && (result.success || result.id)) {
                ...
                toast.error(result?.message || result?.error || 'Ошибка при обновлении товара');
    ...
    const handleDeleteProduct = async () => {
        ...
        if (response && !response.error) {
            ...
            toast.error(response?.message || response?.error || 'Ошибка при удалении товара');
        }
    };
        ...
        <div>
                        ...
                        >
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Название *</label>
                                        <input
                                            type="text"
                                            name="name"
                                            value={formData.name}
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Артикул *</label>
                                        <input
                                            type="text"
                                            name="article"
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Категория *</label>
                                        <select
                                            name="category"
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Цена (₽) *</label>
                                        <input
                                            type="number"
                                            name="price"
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Количество на складе</label>
                                        <input
                                            type="number"
                                            name="stock"
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Ширина (мм)</label>
                                        <input
                                            type="number"
                                            name="width"
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Высота (мм)</label>
                                        <input
                                            type="number"
                                            name="height"
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2 text-[#1E1E1E]">Глубина (мм)</label>
                                        <input
                                            type="number"
                                            name="depth"
                                    ...
                                    <div className="mb-4">
                                        <label className="flex items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-[#2C5AA0] transition-colors">
                                            <div className="text-center">
                                                ...
                                                <p className="text-sm text-gray-600">Загрузите изображения</p>
                                ...
                                <div className="flex items-center">
                                    ...
                                    />
                                    <label htmlFor="is_active" className="ml-2 text-sm font-semibold text-[#1E1E1E]">
                                        Активный товар
                                    </label>
                                </div>
                            </div>
                            <div className="flex gap-3 pt-6">
                                <Button
                                    ...
                                    disabled={loading || !formData.name || !formData.article || formData.price <= 0 || !formData.category || uploadingImages.length === 0}
                                />
                                <Button
            ...
            <AnimatePresence>
                {showEditModal && selectedProduct && (
                    ...
                    >
                        ...
                        >
                            <div className="flex justify-between items-center mb-6">
                                <Title text={"Редактировать товар"} size={"text-xl font-bold"} />
                                <button
                                    onClick={handleCloseEditModal}
                                    className="w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"
                                >
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="mb-4">
                                        <label className="flex items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-[#2C5AA0] transition-colors">
                                            <div className="text-center">
                                                <FiUpload className="mx-auto text-2xl text-gray-400 mb-2" />
                                                <p className="text-sm text-gray-600">Добавить новые изображения</p>
                                                <p className="text-xs text-gray-500">PNG, JPG до 5MB</p>
                                            </div>
                                            <input
                                                type="file"
                                                ...
                                                <div key={index} className="relative group">
                                                    <div className="aspect-square rounded-lg overflow-hidden border border-gray-200">
                                                        {img instanceof File ? (
                                                            ...
                                                            <Image
                                                                src={typeof img === 'object' ? img.image_url : img}
                                ...
                                <div className="flex items-center">
                                    <input
                                        ...
                                        className="w-4 h-4 text-[#2C5AA0] rounded focus:ring-[#2C5AA0]"
                                    />
                                    <label htmlFor="is_active_edit" className="ml-2 text-sm font-semibold text-[#1E1E1E]">
                                        Активный товар
                                    </label>
                                </div>
                            </div>
                            <div className="flex gap-3 pt-6">
                                <Button
                                    ...
                                    disabled={loading || !formData.name || !formData.article || formData.price <= 0 || !formData.category}
                                />
                                <Button
                                    className="flex-1 h-[48px] bg-gray-200 !text-gray-700 hover:bg-gray-300"
                                    text="Отмена"
                    ...
                    >
                        ...
                        >
                            <div className="text-center">
                                ...
                                <h3 className="text-xl font-bold text-[#1E1E1E] mb-2">Удалить товар?</h3>
                                <p className="text-[#1E1E1E]/60 mb-6">
                                    Вы уверены, что хотите удалить товар <span className="font-semibold">"{productToDelete.name}"</span>?
            ...
            <div className="flex justify-between md:items-center max-md:flex-col max-md:gap-4">
                <div className="flex flex-col">
                    <Title text={"Управление товарами"} size={"text-[24px] max-md:text-[22px]"} cls="uppercase" />
            ...
            {products.length === 0 && !isLoadingData ? (
                <div className="mt-6 bg-white rounded-2xl p-8 text-center" style={{ boxShadow: "0px 0px 4px 0px #76767626" }}>
                    ...
                    <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {products.map((product) => (
                            <div key={product.id} className="relative group">
                                <div className="p-4 relative rounded-[12px] transition-all duration-300 w-full max-md:p-2 max-md:rounded-[8px] bg-white hover:shadow-lg"
                                ...
                                >
                                    ...
                                    <div className="w-full aspect-square mb-4 rounded-lg overflow-hidden">
                                        <Image
                                            src={product.images_list?.[0] || product.images?.[0]?.image_url || '/cart.png'}
                                            alt={product.name}
                                            width={300}
                                            height={300}
                                            className="w-full h-full object-cover"
                                    ...
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <div className="text-sm text-[#1E1E1E]/60">
                                            На складе: {product.stock || 0} шт.
                                        </div>
                                    </div>

                                    {/* Description */}
                                    {product.description && (
                                        <div className="text-sm text-[#1E1E1E]/70 mb-4 line-clamp-2">
                                            {product.description}
                                        </div>
                                    )}
                                </div>
