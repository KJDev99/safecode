...
export default function ManagerBills() {
    ...
    const [bills, setBills] = useState([]); // Alohida state
    ...
    const handleCreateBill = async () => {
        ...
        try {
            ...
            if (response && !response.error && (response.success || response.id || response.data)) {
                ...
                resetFormData();
                setSelectedFiles([]);
                await loadBills(); // Yangi funksiyani chaqiramiz
            } else {
    ...
    const openEditModal = (bill) => {
        ...
        setFormData({
            object_id: bill.object_id?.toString() || bill.object?.id?.toString() || '',
    ...
    const getObjectName = (bill) => {
        // Bills API dan kelgan object obyektidan foydalanamiz
        if (bill.object && bill.object.name) {
            return bill.object.name;
        }
        // Yoki agar object_id bo'lsa, userObjects dan qidiramiz
        if (bill.object_id && userObjects.length > 0) {
            const obj = userObjects.find(o => o.id === bill.object_id);
            return obj ? obj.name : 'Неизвестный объект';
        }
        return 'Неизвестный объект';
    };

    const getObjectAddress = (bill) => {
        // Bills API dan kelgan object obyektidan foydalanamiz
        if (bill.object && bill.object.address) {
            return bill.object.address;
        }
        // Yoki agar object_id bo'lsa, userObjects dan qidiramiz
        if (bill.object_id && userObjects.length > 0) {
            const obj = userObjects.find(o => o.id === bill.object_id);
